...
```bash
uv run vv-trans --audio "C:\Users\Serge\Documents\Sound Recordings\my_voice_example_en.mp3"
```

By default the result is written next to the audio file with the suffix `.trans.<runtime>.json`.

Examples (default `--runtime 4`):
...
Three quantization/precision modes are available:

```bash
# 4-bit (default) — lowest VRAM, bitsandbytes NF4 quantization
uv run vv-trans --audio file.mp3 --runtime 4

# 8-bit — bitsandbytes INT8 quantization
uv run vv-trans --audio file.mp3 --runtime 8

# 16-bit — full bfloat16 precision, no quantization
uv run vv-trans --audio file.mp3 --runtime 16
```

Different runtimes produce separate output files (`.trans.4.json`, `.trans.8.json`, `.trans.16.json`), so you can compare them side by side.
...
```bash
uv run vv-trans --audio file.mp3 --runtime 16 --chunk-seconds 30
```
...
```bash
uv run vv-trans --audio "C:\Users\Serge\Documents\Sound Recordings"
```
...
uv run vv-trans --audio ./audio_dir/ --force

# Batch with specific runtime
uv run vv-trans --audio ./audio_dir/ --runtime 16 --chunk-seconds 30
```
...
```bash
uv run vv-trans --audio file.mp3 --output "results/custom.json"
```
...
```bash
uv run vv-trans --audio interview.mp3 --prompt "Interview between two speakers"
```
